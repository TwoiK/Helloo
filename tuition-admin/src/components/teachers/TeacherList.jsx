...
const fetchData = async (showLoadingState = true) => {
  ...
  // Add this function to handle application status updates
const handleApplicationStatus = async (applicationId, newStatus) => {
    try {
        await apiService.updateVacancyApplication(applicationId, newStatus);
        message.success(`Application ${newStatus} successfully`);
        
        // Refresh applicants list if there's a selected vacancy
        if (selectedVacancy?._id) {
            const response = await apiService.getVacancyApplicants(selectedVacancy._id);
            setSelectedVacancyApplicants(response.data || []);
        }
    } catch (error) {
